...
```bash
mkdir -p archived && mv logs/old/* archived/
```
...
```bash
ls -S logs/ | grep -v / | head -1 > biglog.txt
```
...
find logs/config/ -name "*.conf" -exec cp --parents {} backup_temp/ \;
find logs/ -name "*.logs" -exec cp --parents {} backup_temp/ \;

if [ -f "errors.txt" ]; then
    cp errors.txt backup_temp/
fi

zip -r project_backup.zip backup_temp/
...
```bash
wc -l logs/config/*.conf | head -n -1 | awk '{print $2, $1}' > conf_stats.txt
```
